...
async function startScan() {
  ...
  try {
    // Serialize the vendor scraper function and run it in the page context
    const scraperSource = state.vendor.scraper.toString();

    const results = await chrome.scripting.executeScript({
      target: { tabId: state.tabId },
      func: new Function(`return (${scraperSource})()`)
    });
